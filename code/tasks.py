...
from AesUST.test import testAesUST
...
def optimize_params(effect, preset, content, style, result_image_placeholder):
    ...
    # strotss 是另一篇paper的 style transfer (或許這個還蠻重要的?)
    # 這個reference就已經是style tranfered image
    if st.session_state['transferMode'] == "STROTSS":
    ...
    # 該paper的部分
    # vp應該是VisualParameter (parameter的tensor，我猜)
    # content_img_cuda應該是以torch表示的img
