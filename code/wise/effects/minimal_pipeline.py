...
class MinimalPipelineEffect(EffectBase):
    def __init__(self, **kwargs):
        ...
        # effects
        # 從RGB表示法無法得知像素的明亮度(luminance)、飽和度(saturation)及色相(hue)
        # CIELAB色彩空間的表示法為(L*, a*, b*)，L*代表黑色到白色間的位置，也就是明亮度(luminance)，範圍是0~100
        # a*為綠色(green)到紫紅色(magenta)間的位置，範圍是-500~500
        # b*為藍色(blue)到黃色(yellow)間的位置，範圍是-200~200
        # 距離圓心愈近飽和度愈底
